...
export async function slash(client: ToolClient, interaction: ChatInputCommandInteraction) {
    ...
    const perkFlags = {
        ...
        BugHunterLevel1: `Chasseur de bugs (Level 1)`,
        BugHunterLevel2: `Chasseur de bugs (Level 2)`,
...
**» Badges:** ${userFlags.length > 0 ? `\`${userFlags.map((flag: any) => perkFlags[<keyof object>flag]).join(', ')}\`` : '`Aucun`'}
